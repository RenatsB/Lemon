...
void main() {
    ...
    //this bit is supposed to change bump orientation at the top
    if(FragmentTexCoord.y>1.225f && FragmentTexCoord.y<1.235f)
    {
      bump = normalize(vec4(RawPosition.x,0.0,RawPosition.z,1.0));
...
//      texColor = smoothstep(texColor, white, vec3((0.1f - distance(RawPosition.xz,vec2(0)))*5.f));
